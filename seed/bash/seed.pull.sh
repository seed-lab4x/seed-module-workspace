...
if [[ ! -f $subrepo_file ]];
then
    echo "Need to clone subrepo '$subrepo_subdir' <- $subrepo_remote $subrepo_branch"
    git subrepo clone \
        $subrepo_remote \
        $subrepo_subdir \
...
then
    ...
    then
        echo "!!!!!!!!!!"
        echo "The actual pull remote is different from the .gitrepo fix later."

        git subrepo pull \
            --message="git subrepo pull --remote=$subrepo_remote --branch=$subrepo_branch $subrepo_subdir" \
